...
export default async function ProfilePage() {
  ...
  const favoriteProducts =
    myFavorites?.map((f) => f.products).filter((p) => p !== null) || []
  const favoritedProductIds = new Set(
    myFavorites?.map((f) => f.product_id) || []
  )
      ...
      <Tabs defaultValue='listings' className='w-full'>
        ...
        <TabsContent value='listings' className='mt-6'>
          {myListings && myListings.length > 0 ? (
            <div className='grid grid-cols-2 gap-3 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4 xl:grid-cols-5'>
              {myListings.map((product) => (
                <div key={product.id} className='group relative'>
                  ...
                  {product.status === 'sold' && (
                    <div className='absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-gray-900/50 backdrop-blur-[1px]'>
                      <span className='rotate-[-12deg] rounded border-4 border-red-500 px-4 py-2 text-xl font-black text-red-500 opacity-80 shadow-2xl'>
                        已下架
                      </span>
                    </div>
                  )}
